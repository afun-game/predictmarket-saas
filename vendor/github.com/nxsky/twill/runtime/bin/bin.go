...
// Package bin contains code to extract data from a Twill binary.
package bin
...
import (
	...
	"github.com/nxsky/twill/runtime/graph"
	"github.com/nxsky/twill/runtime/version"
	"golang.org/x/exp/maps"
	"golang.org/x/exp/slices"
)

// versionData exists to embed the twill module version and deployer API
// version into a Twill binary. We split declaring and assigning
...
func ReadComponentGraph(file string) ([]string, graph.Graph, error) {
	...
	const mainComponent = "github.com/nxsky/twill/Main"
...
type Versions struct {
	ModuleVersion   string         // Twill library's module version
...
// extractModuleVersion returns the version of the Twill library
// embedded in data.
func extractModuleVersion(filename string) (string, error) {
	...
	// Find the Twill module.
	const weaverModule = "github.com/nxsky/twill"
	for _, m := range append(info.Deps, &info.Main) {
		if m.Path == weaverModule {
			return m.Version, nil
		}
	}
	return "", fmt.Errorf("Twill module was not linked into the application binary: that's an error")
}
