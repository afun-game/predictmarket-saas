...
// Register registers a Twill component.
...
// registry is a repository for registered Twill components.
// Entries are typically added to the default registry by calls
// to Register in init functions in code generated by "twill generate".
...
// Registration is the configuration needed to register a Twill component.
type Registration struct {
	...
	Listeners []string     // the names of any twill.Listeners
	...
	// RefData holds generated reference metadata for this component, including
	// component edges, listeners, and adapter declarations.
	RefData string
}

// register registers a Twill component. If the registry's close method was
...
func (r *registry) register(reg Registration) error {
	...
	if old, ok := r.byName[reg.Name]; ok {
		return fmt.Errorf("component name %s already registered for type %v when registering %v",
			reg.Name, old.Iface, reg.Iface)
	}
	ptr := &reg
...
// allComponents returns copies of all registered components.
func (r *registry) allComponents() []*Registration {
	...
	for _, info := range r.components {
		components = append(components, cloneRegistration(info))
	}
...
func cloneRegistration(reg *Registration) *Registration {
	clone := *reg
	clone.Listeners = slices.Clone(reg.Listeners)
	clone.NoRetry = slices.Clone(reg.NoRetry)
	return &clone
}
...
func ComponentConfigValidator(path, cfg string) error {
	...
	if componentConfig == nil {
		return fmt.Errorf("unexpected configuration for component %v "+
			...
			"twill.WithConfig[configType] embedded field to %v)",
...
func CallGraph() []CallEdge {
	...
	for _, reg := range Registered() {
		...
		for i, n := 0, impl.NumField(); i < n; i++ {
			// Handle field with type twill.Ref[T].
			ref := impl.Field(i).Type
			if ref.PkgPath() == "github.com/nxsky/twill" &&
