...
// A Stub allows a Twill component in one process to invoke methods
// via RPC on a Twill component in a different process.
type Stub interface {
...
// A Server allows a Twill component in one process to receive and execute
// methods via RPC from a Twill component in a different process. It is the
// dual of a Stub.
type Server interface {
	// GetStubFn returns a handler function for the given method. For example,
	// if a Twill component defined an Echo method, then GetStubFn("Echo")
