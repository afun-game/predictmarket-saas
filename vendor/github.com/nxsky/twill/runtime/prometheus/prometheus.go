...
// TranslateMetricsToPrometheusTextFormat translates Twill
...
func TranslateMetricsToPrometheusTextFormat(w *bytes.Buffer, ms []*metrics.MetricSnapshot, lisAddr, path string) {
	...
	// Display the user metrics first, followed by the Twill
	...
	for _, m := range ms {
		if strings.HasPrefix(m.Name, "servicetwill_") {
	...
	if len(weaverMetrics) > 0 {
		fmt.Fprintf(w, "# ┌─────────────────────────────────────┐\n")
		fmt.Fprintf(w, "# │ SERVICETWILL AUTOGENERATED METRICS │\n")
		fmt.Fprintf(w, "# └─────────────────────────────────────┘\n\n")
	}
...
#
# scrape_configs:
# - job_name: 'prometheus-servicetwill-scraper'
#   scrape_interval: 5s
#   metrics_path: %s
#   static_configs:
#     - targets: ['%s']
...
// translateMetrics translates a slice of metrics from the Twill format
...
func translateMetrics(w *bytes.Buffer, metrics []*metrics.MetricSnapshot) string {
	...
	for idx, metric := range metrics {
		...
		delete(labels, "servicetwill_app")
		delete(labels, "servicetwill_version")
		if node, ok := labels["servicetwill_node"]; ok {
			labels["servicetwill_node"] = logging.Shorten(node)
		}
