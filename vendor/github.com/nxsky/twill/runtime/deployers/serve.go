...
// Package deployers defines Twill's small public deployer extension surface.
//
// Planner implementations produce deterministic, read-only deployment plans.
// They must not modify files, read or write deployment environments, contact
// registries, query cloud APIs, or apply manifests. Plan values should carry
// explicit schema versions, dry-run/write markers, limitations, and local
// verification commands so CLI, MCP, dashboard, and future third-party
// deployer integrations can review a plan before any apply flow exists.
//
// The ServeComponents and NewUnixSocketPath helpers are retained for
// compatibility with existing deployer code. New deployment integrations should
// prefer Planner and the generic Plan schema when exposing reviewable rollout
// metadata.
package deployers
