...
type LogsSpec struct {
	Tool    string                                        // tool name, e.g., "twill gke"
...
Queries:
  Every log entry has the following fields:
      ...
      * component      : the abbreviated Twill component name
      * full_component : the unabbreviated Twill component name
...
Query Reference:
  ...
  [1]: https://opensource.google/projects/cel`
	...
	return &Command{
		Name:        "logs",
		Flags:       spec.Flags,
		Description: "Cat or follow Twill logs",
		Help:        b.String(),
		Fn:          spec.logFn,
	}
}
...
func (s *LogsSpec) logFn(ctx context.Context, args []string) error {
	...
	if !s.system {
		query += ` && !("servicetwill/system" in attrs)`
	}
