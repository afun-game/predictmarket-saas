...
import (
	...
	"github.com/nxsky/twill/internal/must"
	"github.com/nxsky/twill/internal/status"
	itool "github.com/nxsky/twill/internal/tool"
	...
	dashboardSpec = &status.DashboardSpec{
		...
		Commands: func(deploymentId string) []status.Command {
			return []status.Command{
				{Label: "status", Command: "twill single status"},
				{Label: "profile", Command: fmt.Sprintf("twill single profile --duration=30s %s", deploymentId)},
			}
		},
	}
	purgeSpec = &tool.PurgeSpec{
		Tool:  "twill single",
		Kill:  "twill single (dashboard|profile)",
		Paths: []string{dataDir},
	}

	Commands = map[string]*tool.Command{
		"deploy":    &deployCmd,
		"status":    status.StatusCommand("twill single", defaultRegistry),
		"dashboard": status.DashboardCommand(dashboardSpec),
		"metrics":   status.MetricsCommand("twill single", defaultRegistry),
		"profile":   status.ProfileCommand("twill single", defaultRegistry),
		"purge":     tool.PurgeCmd(purgeSpec),
		"version":   itool.VersionCmd("twill single"),
	}
)
