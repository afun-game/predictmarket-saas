...
func formatMetrics(metrics []*protos.MetricSnapshot) {
	...
	for _, metric := range metrics {
		if strings.HasPrefix(metric.Name, "servicetwill_system") {
			// Ignore Twill internal metrics.
			continue
		}
	...
	internal := func(name string) bool {
		return strings.HasPrefix(name, "servicetwill")
	}
	...
	for _, group := range groups {
		...
		for _, m := range group {
			...
			for _, key := range keys {
				...
				case key == "servicetwill_node":
					entry.S = logging.Shorten(entry.S)
				case key == "servicetwill_version":
