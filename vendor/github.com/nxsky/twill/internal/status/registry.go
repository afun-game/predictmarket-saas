...
// "twill multi status" and "twill single dashboard".
package status
...
import (
	...
	"github.com/nxsky/twill/internal/files"
	"github.com/nxsky/twill/runtime/colors"
)

// A Registry is a persistent collection of Twill application metadata.
//
// Tools like "twill multi status" and "twill multi dashboard" use the registry
...
// A Registration contains basic metadata about a Twill application.
...
func (r *Registry) List(ctx context.Context) ([]Registration, error) {
	...
	for _, reg := range regs {
		// When a Twill application is deployed, it registers itself
