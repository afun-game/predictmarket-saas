...
// TODO(mwhittaker): Right now, a user has to (1) run `twill profile` to get a
// profile and then (2) pass it to the `go pprof` command. Srdjan suggested we
// have `twill profile` wrap `pprof` to make this a single step instead of two.
...
Examples:
  TODO(rgrandl): Show how to take a profile and pass it to pprof.`
	...
	return &tool.Command{
		Name:        "profile",
		Description: "Profile a running Twill application",
		Help:        b.String(),
		Flags:       profileFlags,
		Fn: func(ctx context.Context, args []string) error {
			...
			name := fmt.Sprintf("servicetwill_%s_%s_profile_*.pb.gz", status.App, *profileType)
