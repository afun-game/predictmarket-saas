...
import (
	...
	metrics2 "github.com/nxsky/twill/internal/metrics"
	"github.com/nxsky/twill/internal/traceio"
	"github.com/nxsky/twill/runtime/logging"
	"github.com/nxsky/twill/runtime/metrics"
	"github.com/nxsky/twill/runtime/perfetto"
	imetrics "github.com/nxsky/twill/runtime/prometheus"
	protos "github.com/nxsky/twill/runtime/protos"
	dtool "github.com/nxsky/twill/runtime/tool"
...
var (
	...
	dashboardPort   = dashboardFlags.Int("port", 0, "Dashboard port")
	dashboardAppDir = dashboardFlags.String(
		"app-dir",
		".",
		"Application directory for local dashboard context",
	)
	dashboardAppPackages = dashboardFlags.String(
		"app-packages",
		".",
		"Comma or space separated Go package patterns for local dashboard context",
	)
	...
	appHTML     string
	appTemplate = template.Must(template.New("app").Parse(appHTML))
...
type Command struct {
	...
	Command string // e.g., twill single logs '--version=="12345678"'
}

// DashboardSpec configures the command returned by DashboardCommand.
type DashboardSpec struct {
	Tool         string                                   // tool name (e.g., "twill single")
...
{{.Flags}}`
	...
	return &dtool.Command{
		Name:        "dashboard",
		Description: "Inspect Twill applications",
		Help:        b.String(),
		...
		Fn: func(ctx context.Context, _ []string) error {
			...
			return http.Serve(lis, dashboard.handler())
		},
	}
}

// dashboard implements the "twill dashboard" HTTP server.
type dashboard struct {
	spec     *DashboardSpec // e.g., "twill multi" or "twill single"
...
func (d *dashboard) handler() http.Handler {
	...
	mux.HandleFunc("/app", d.handleApp)
	mux.HandleFunc("/app/data", d.handleAppData)
	mux.HandleFunc("/favicon.ico", http.NotFound)
	mux.HandleFunc("/deployment", d.handleDeployment)
	mux.HandleFunc("/metrics", d.handleMetrics)
	mux.HandleFunc("/traces", d.handleTraces)
	mux.HandleFunc("/tracefetch", d.handleTraceFetch)
