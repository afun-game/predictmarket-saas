...
import (
	...
	"github.com/nxsky/twill/internal/config"
	"github.com/nxsky/twill/internal/control"
	"github.com/nxsky/twill/internal/net/call"
	"github.com/nxsky/twill/internal/register"
	"github.com/nxsky/twill/internal/traceio"
	"github.com/nxsky/twill/runtime"
	"github.com/nxsky/twill/runtime/codegen"
	"github.com/nxsky/twill/runtime/deployers"
	"github.com/nxsky/twill/runtime/logging"
	"github.com/nxsky/twill/runtime/metrics"
	"github.com/nxsky/twill/runtime/protos"
	"github.com/nxsky/twill/runtime/retry"
...
//
// RemoteWeavelet must implement the twill.controller component interface.
...
// component represents a Twill component and all corresponding
// metadata.
type component struct {
	...
	initGoroutine atomic.Int64   // goroutine ID inside implInit.Do (0 if not initializing)
...
// goroutineID returns the ID of the current goroutine. This is used to detect
// circular dependencies: if a component's initGoroutine matches the current
// goroutine ID, we know we're inside a re-entrant initComponent call.
func goroutineID() int64 {
	b := make([]byte, 64)
	b = b[:stdruntime.Stack(b, false)]
...
func NewRemoteWeavelet(ctx context.Context, regs []*codegen.Registration, bootstrap runtime.Bootstrap, opts RemoteWeaveletOptions) (*RemoteWeavelet, error) {
	...
	// Set up logging.
	w.syslogger = w.logger("weavelet", "servicetwill/system", "")
...
// InitWeavelet implements twill.controller and conn.WeaverHandler interfaces.
...
// getIntfForRef is like getIntf but uses getImplForRef for local components,
// allowing circular dependency resolution during fillRefs. This is safe
// because refs are only stored for later use; methods are not called during
// initialization.
func (w *RemoteWeavelet) getIntfForRef(t reflect.Type, requester string) (any, error) {
	c, ok := w.componentsByIntf[t]
	if !ok {
		return nil, fmt.Errorf("component of type %v was not registered; maybe you forgot to run twill generate", t)
	}
	...
	// Return a local stub.
	if c.local.Read() {
		impl, err := w.getImplForRef(c.reg.Impl)
		if err != nil {
			return nil, err
		}
...
//
// GetImpl blocks until the component is fully initialized (refs filled, Init
// called). For circular dependency resolution during fillRefs, use
// getImplForRef instead, which may return a pre-cached instance.
func (w *RemoteWeavelet) GetImpl(t reflect.Type) (any, error) {
	c, ok := w.componentsByImpl[t]
	if !ok {
		return nil, fmt.Errorf("component implementation of type %v was not registered; maybe you forgot to run twill generate", t)
	}
	w.initComponent(c)
	return c.impl, c.implErr
}

// getImplForRef returns the component implementation for ref resolution
// during fillRefs. Unlike GetImpl, it may return a pre-cached (not yet fully
// initialized) instance to break circular dependency cycles. This is safe
// because refs are only stored for later use; methods are not called during
// fillRefs.
func (w *RemoteWeavelet) getImplForRef(t reflect.Type) (any, error) {
	c, ok := w.componentsByImpl[t]
	if !ok {
		return nil, fmt.Errorf("component implementation of type %v was not registered; maybe you forgot to run twill generate", t)
	}
	// If the current goroutine is inside implInit.Do for this component
	// (circular dependency), return the pre-cached impl immediately.
	// This avoids a sync.Once re-entrant deadlock. For non-circular
	// dependencies (different goroutine), we fall through to
	// initComponent which blocks on implInit.Do until initialization
	// completes.
	if c.initGoroutine.Load() == goroutineID() && c.impl != nil {
		return c.impl, nil
	}
	w.initComponent(c)
	return c.impl, c.implErr
...
// initComponent runs the implInit.Do function to create and initialize the
// component. It is safe to call from multiple goroutines.
func (w *RemoteWeavelet) initComponent(c *component) {
	c.implInit.Do(func() {
		// Track the goroutine inside implInit.Do for circular
		// dependency detection in getImplForRef.
		c.initGoroutine.Store(goroutineID())
		defer c.initGoroutine.Store(0)

		name := logging.ShortenComponent(c.reg.Name)
		w.syslogger.Debug("Constructing", "component", name)

		// If a fake is registered, use it directly. Fakes don't need
		// ref filling or initialization.
		if fake, ok := w.opts.Fakes[c.reg.Iface]; ok {
			c.impl = fake
			c.serverStub = c.reg.ServerStubFn(c.impl, func(uint64, float64) {})
			c.implReady.Store(true)
			w.syslogger.Debug("Constructed (fake)", "component", name)
			return
		}

		// Create the raw instance and pre-set c.impl before filling
		// refs. This breaks circular dependency cycles: a re-entrant
		// getImplForRef call from fillRefs will find c.impl already set
		// and return it via the early return, avoiding a sync.Once
		// deadlock.
		v := reflect.New(c.reg.Impl)
		c.impl = v.Interface()

		// Set up serverStub before finishComponent, because getImplForRef
		// may allow method handlers to see c.impl before finishComponent
		// completes. Method handlers use c.serverStub to invoke methods.
		// Method handlers call GetImpl (not getImplForRef), which blocks
		// on implInit.Do until initialization completes.
		...
		c.implErr = w.finishComponent(w.ctx, c.impl, c.reg)
		if c.implErr != nil {
...
func (w *RemoteWeavelet) createComponent(ctx context.Context, reg *codegen.Registration) (any, error) {
	...
	if err := w.finishComponent(ctx, obj, reg); err != nil {
		return nil, err
	}
	return obj, nil
}

// finishComponent fills config, logger, refs, listeners, and calls Init
// on a pre-created component instance. It is called by GetImpl after
// pre-setting c.impl to break circular dependency cycles.
//
// NOTE: Init methods should only initialize the component's own state
// (open database connections, set up caches, etc.). They should NOT
// call methods on other components via Ref.Get(), because those
// components may not have completed their own Init yet when circular
// dependencies are involved.
func (w *RemoteWeavelet) finishComponent(ctx context.Context, obj any, reg *codegen.Registration) error {
	if _, ok := w.opts.Fakes[reg.Iface]; ok {
		return nil
	}
	...
	// Fill ref fields.
	// Use getIntfForRef which allows returning pre-cached instances
	// to break circular dependency cycles.
	if err := FillRefs(obj, func(t reflect.Type) (any, error) {
		return w.getIntfForRef(t, reg.Name)
	}); err != nil {
		return err
	}
...
// UpdateComponents implements twill.controller and conn.WeaverHandler interfaces.
...
func (w *RemoteWeavelet) GetMetrics(context.Context, *protos.GetMetricsRequest) (*protos.GetMetricsReply, error) {
	...
	for _, def := range updates.Defs {
		...
		def.Labels["servicetwill_app"] = w.Info().App
		def.Labels["servicetwill_version"] = w.Info().DeploymentId
		def.Labels["servicetwill_node"] = w.Info().Id
	}
...
func (w *RemoteWeavelet) getComponent(name string) (*component, error) {
	...
	if !ok {
		return nil, fmt.Errorf("component %q was not registered; maybe you forgot to run twill generate", name)
	}
	return c, nil
}
...
func (w *RemoteWeavelet) addHandlers(handlers *call.HandlerMap, c *component) {
	for i, n := 0, c.reg.Iface.NumMethod(); i < n; i++ {
		...
		handler := func(ctx context.Context, args []byte) (res []byte, err error) {
			...
			// component has already been started. GetImpl blocks on
			// implInit.Do until the component is fully initialized (refs
			// filled, Init called), so it is safe to invoke methods after it
			// returns.
			if _, err := w.GetImpl(c.reg.Impl); err != nil {
				return nil, err
			}
	...
	// (see waitUntilReady).
	//
	// We return success when c.impl is non-nil (pre-cached by GetImpl) even if
	// c.implReady is not yet set. This breaks circular dependency deadlocks
	// where two components on different processes wait for each other to become
	// ready. Method calls through the stub are still safe: the method handler
	// calls GetImpl which blocks on implInit.Do until initialization completes.
