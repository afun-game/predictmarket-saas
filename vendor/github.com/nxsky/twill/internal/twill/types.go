...
var (
	...
	// twill package. The circuitousness is to avoid cyclic dependencies.
	...
	// an embedded twill.Config field.
	...
	// twill.Database, twill.Cache, or other resource fields.
	HasResources func(impl any) bool

	// FillResources initializes resource fields in a component implementation
	// struct.
	//   - impl should be a pointer to the implementation struct
	//   - get should be a function that returns the resource handle for the
	//     requested name and kind (e.g., "database" or "cache").
	FillResources func(impl any, get func(name, kind string) (any, error)) error
)

// Copy of the same struct in the main twill package.
