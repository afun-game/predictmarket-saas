...
import (
	...
	"github.com/nxsky/twill/internal/env"
	imetrics "github.com/nxsky/twill/internal/metrics"
	"github.com/nxsky/twill/internal/status"
	"github.com/nxsky/twill/internal/tool/single"
	"github.com/nxsky/twill/internal/traceio"
	"github.com/nxsky/twill/runtime"
	"github.com/nxsky/twill/runtime/codegen"
	"github.com/nxsky/twill/runtime/colors"
	runtimeconfig "github.com/nxsky/twill/runtime/config"
	...
	"github.com/nxsky/twill/runtime/traces"
...
// process. It is the weavelet used when you "go run" a Twill app.
type SingleWeavelet struct {
	...
	// Loader resolves config from TOML sections and environment variables.
	loader runtimeconfig.Loader

	// Resource manager resolves database, cache, and other resource handles.
...
func NewSingleWeavelet(ctx context.Context, regs []*codegen.Registration, opts SingleWeaveletOptions) (*SingleWeavelet, error) {
	...
	// Build layered config loader: TOML → file (K8s ConfigMap/Secret) → env.
	// File loaders are enabled when TWILL_CONFIG_DIR or TWILL_SECRET_DIR
	// environment variables are set, supporting the standard Kubernetes
	// pattern of mounting ConfigMaps and Secrets as directories of files.
	loaders := []runtimeconfig.Loader{
		runtimeconfig.NewTomlLoader(config.App.Sections),
	}
	if dir := os.Getenv("TWILL_CONFIG_DIR"); dir != "" {
		loaders = append(loaders, runtimeconfig.NewFileLoader(dir))
	}
	if dir := os.Getenv("TWILL_SECRET_DIR"); dir != "" {
		loaders = append(loaders, runtimeconfig.NewFileLoader(dir))
	}
	loaders = append(loaders, runtimeconfig.NewEnvLoader("TWILL_"))
	loader := runtimeconfig.NewLayered(loaders...)
	w := &SingleWeavelet{
		ctx:          ctx,
		regs:         regs,
		...
		loader:       loader,
		resourceManager: resource.NewManager(resource.NewLayeredResolver(
			resource.NewLoaderResolver(loader),
			resource.NewEnvResolver("TWILL_RESOURCE_"),
		)),
...
func (w *SingleWeavelet) getIntf(t reflect.Type, requester string) (any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("component %v not found; maybe you forgot to run twill generate", t)
	}
	c, err := w.get(reg)
	if err != nil {
		return nil, err
...
func (w *SingleWeavelet) getImpl(t reflect.Type) (any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("component implementation %v not found; maybe you forgot to run twill generate", t)
	}
	return w.get(reg)
}
...
func (w *SingleWeavelet) get(reg *codegen.Registration) (any, error) {
	...
	// Pre-register the instance in the component cache before filling
	// refs, listeners, or calling Init. This breaks circular dependency
	// cycles: if component A depends on B and B depends on A, B's fillRefs
	// will find A already in the cache and receive a valid (though not
	// yet fully initialized) reference. The reference becomes fully
	// usable once all components have completed initialization.
	...
	if err := FillResources(obj, func(name, kind string) (any, error) {
		ctx := context.Background()
		switch kind {
		case "database":
			return w.resourceManager.ResolveDatabase(ctx, name)
		case "cache":
			return w.resourceManager.ResolveCache(ctx, name)
		case "pubsub":
			return w.resourceManager.ResolvePubSub(ctx, name)
		case "cron":
			return w.resourceManager.ResolveCron(ctx, name)
		case "secret":
			return w.resourceManager.ResolveSecret(ctx, name)
		default:
	...
	// dependencies are involved. Cross-component calls are safe after
...
func (w *SingleWeavelet) listener(name string) (net.Listener, error) {
	...
	// Load config-based middleware for the listener from the unified loader.
	middlewares, err := middleware.LoadFromLoader(w.ctx, w.loader, name)
	if err != nil {
		return nil, fmt.Errorf("listener %q: %w", name, err)
	}
	ml := middlewareListener{Listener: lis, middleware: middlewares}
...
// middlewareListener wraps a net.Listener with a pre-resolved middleware chain.
// It is used by the singleprocess runtime to carry listener middleware without
// creating a package import cycle with the public twill package.
type middlewareListener struct {
	net.Listener
	middleware []middleware.Middleware
}

// MiddlewareChain implements middleware.MiddlewareListener.
func (l middlewareListener) MiddlewareChain() []middleware.Middleware {
	return l.middleware
}
...
func (w *SingleWeavelet) Metrics(context.Context) (*status.Metrics, error) {
	...
	for _, snap := range metrics.Snapshot() {
		...
		proto.Labels["servicetwill_app"] = w.config.App.Name
		proto.Labels["servicetwill_version"] = w.deploymentId
		proto.Labels["servicetwill_node"] = w.id
