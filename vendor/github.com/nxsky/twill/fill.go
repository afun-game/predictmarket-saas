...
func init() {
	// See internal/twill/types.go.
	twill.SetLogger = setLogger
	twill.SetWeaverInfo = setWeaverInfo
	twill.HasRefs = hasRefs
	twill.FillRefs = fillRefs
	twill.HasListeners = hasListeners
	twill.FillListeners = fillListeners
	twill.HasConfig = hasConfig
	twill.GetConfig = getConfig
	twill.HasResources = HasResources
	twill.FillResources = FillResources
}
...
func setLogger(v any, logger *slog.Logger) error {
	...
	if !ok {
		return fmt.Errorf("setLogger: %T does not implement twill.Implements", v)
	}
	x.setLogger(logger)
	return nil
}
...
// See internal/twill/types.go.
func setWeaverInfo(impl any, info *twill.WeaverInfo) error {
	x, ok := impl.(interface{ setWeaverInfo(*twill.WeaverInfo) })
	if !ok {
		return fmt.Errorf("setWeaverInfo: %T does not implement twill.Implements", impl)
	}
...
func fillListeners(impl any, get func(name string) (net.Listener, string, error)) error {
	...
	for i, n := 0, s.NumField(); i < n; i++ {
		...
		if tl, ok := lis.(Listener); ok {
			// Preserve the Twill listener (name, middleware) returned by the
			// caller, but set the proxy address from the caller.
			*l = tl
			l.proxyAddr = proxyAddr
		} else if ml, ok := lis.(middleware.MiddlewareListener); ok {
			// Wrap a listener carrying a middleware chain into a Twill listener,
			// preserving the chain and the caller's proxy address.
			*l = NewListener(lis, name, ml.MiddlewareChain()...)
			l.proxyAddr = proxyAddr
		} else {
			l.Listener = lis
			l.proxyAddr = proxyAddr
