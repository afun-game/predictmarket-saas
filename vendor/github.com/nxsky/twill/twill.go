...
// Package twill provides the interface for building
// [single-image distributed programs].
//
// A program is composed of a set of Go interfaces called
// components. Components are recognized by "twill generate" (typically invoked
// via "go generate"). "twill generate" generates code that allows a component
// to be invoked over the network. This flexibility allows Twill
// to decompose the program execution across many processes and machines.
//
// [single-image distributed programs]: https://github.com/nxsky/twill
package twill
...
import (
	...
	"github.com/nxsky/twill/internal/twill"
...
//go:generate ./cmd/twill/twill generate . ./internal/tool/multi
...
//	err := foo.Foo(ctx)
//	if errors.Is(err, twill.RemoteCallError) {
...
// example.
var RemoteCallError = errors.New("Twill remote call error")
...
// HealthzURL is the URL path on which Twill performs health checks.
// Every application HTTP server must register a handler for this URL path,
// e.g.:
//
//	mux := http.NewServeMux()
//	mux.HandleFunc(twill.HealthzURL, func(http.ResponseWriter, *http.Request) {
//	    // ...
//	})
//
// As a convenience, Twill registers HealthzHandler under
// this URL path in the default ServerMux, i.e.:
//
//	http.HandleFunc(twill.HealthzURL, twill.HealthzHandler)
const HealthzURL = "/debug/twill/healthz"
...
// (i.e. T is a struct that embeds twill.Implements[twill.Main]).
type PointerToMain[T any] interface {
	*T
	InstanceOf[Main]
}

// Run runs app as a Twill application.
//
// The application is composed of a set of components that include twill.Main
// as well as any components transitively needed by twill.Main. An instance
// that implement twill.Main is automatically created by twill.Run and passed
// to app. Note: other replicas in which twill.Run is called may also create
// instances of twill.Main.
//
// The type T must be a struct type that contains an embedded
// `twill.Implements[twill.Main]` field. A value of type T is created,
...
//
// If this process is hosting the `twill.Main` component, Run will call app
...
//
//	func main() {
//	    if err := twill.Run(context.Background(), app); err != nil {
//	        log.Fatal(err)
//	    }
//	}
...
func runLocal[T any, _ PointerToMain[T]](ctx context.Context, app func(context.Context, *T) error) error {
	// Read config from SERVICETWILL_CONFIG env variable, if non-empty.
	opts := twill.SingleWeaveletOptions{}
	if filename := os.Getenv("SERVICETWILL_CONFIG"); filename != "" {
	...
	wlet, err := twill.NewSingleWeavelet(ctx, regs, opts)
	if err != nil {
		return err
	}
...
func runRemote[T any, _ PointerToMain[T]](ctx context.Context, app func(context.Context, *T) error, bootstrap runtime.Bootstrap) error {
	...
	opts := twill.RemoteWeaveletOptions{}
	wlet, err := twill.NewRemoteWeavelet(ctx, regs, bootstrap, opts)
	if err != nil {
		return err
	}
...
var (
	// Equivalence checks with the struct in internal/twill/types.go.
	_ = WeaverInfo(twill.WeaverInfo{})
	_ = twill.WeaverInfo(WeaverInfo{})
)
...
// Twill returns runtime information about the deployed application.
func (i Implements[T]) Twill() WeaverInfo {
	return WeaverInfo(*i.weaverInfo)
}

func (i *Implements[T]) setWeaverInfo(info *twill.WeaverInfo) {
	i.weaverInfo = info
}

// implements is a method that can only be implemented inside the twill
...
// struct. T must be a component type. Twill will automatically
...
// component implementation struct. Once placed, Twill automatically
// initializes the Listener and makes it suitable for receiving network
// traffic. For example:
//
//	type myComponentImpl struct {
//	    twill.Implements[MyComponent]
//	    myListener      twill.Listener
//	    myOtherListener twill.Listener
//	}
//
...
// adding a `twill:"name"` struct tag to their listener fields, e.g.:
//
//	type myComponentImpl struct {
//	    twill.Implements[MyComponent]
//	    myListener      twill.Listener
//	    myOtherListener twill.Listener `twill:"mylistener2"`
//	}
//
...
// renamed using the `twill:"name"` struct tag.
//
...
//
// A listener can carry a pre-resolved middleware chain. Use twill.Serve or
// twill.ServeContext to serve HTTP with the chain applied automatically.
...
// NewListener returns a Twill listener wrapping lis with optional middleware.
// name is used for config-based middleware resolution and diagnostics.
func NewListener(lis net.Listener, name string, middleware ...middleware.Middleware) Listener {
	return Listener{Listener: lis, name: name, middleware: middleware}
}
...
// MiddlewareChain returns the middleware chain configured for this listener.
func (l Listener) MiddlewareChain() []middleware.Middleware {
	return l.middleware
}
...
// Serve serves handler on the listener with the configured middleware chain.
func (l Listener) Serve(handler http.Handler) error {
	return Serve(l, handler)
}

// ServeContext serves handler on the listener with the configured middleware
// chain until ctx is canceled.
func (l Listener) ServeContext(ctx context.Context, handler http.Handler) error {
	return ServeContext(ctx, l, handler)
}

// Serve is a convenience function that serves handler on lis.
// If lis implements middleware.MiddlewareListener, the configured middleware
// chain is applied.
func Serve(lis net.Listener, handler http.Handler) error {
	return ServeContext(context.Background(), lis, handler)
}

// ServeContext serves handler on lis with the configured middleware chain until
// ctx is canceled. If lis implements middleware.MiddlewareListener, the
// configured chain is applied.
func ServeContext(ctx context.Context, lis net.Listener, handler http.Handler) error {
	if ml, ok := lis.(middleware.MiddlewareListener); ok {
		if mws := ml.MiddlewareChain(); len(mws) > 0 {
			handler = middleware.Chain(mws...)(handler)
		}
	}
...
// HTTPAdapter marks an existing net/http route adapted into a Twill listener.
//
// Declare HTTPAdapter fields in a component implementation alongside a
// twill.Listener. The field must have a twill tag with listener, method, and
// path entries:
//
//	type legacy struct {
//	    twill.Implements[Legacy]
//	    public  twill.Listener
//	    profile twill.HTTPAdapter `twill:"listener=public,method=GET,path=/profiles/{id}"`
//	}
//
// The marker is used by twill generate and local app context only. Use
// runtime/adapters to serve existing http.Handler values on the listener.
type HTTPAdapter struct{}

// GRPCAdapter marks an existing gRPC unary method adapted into a Twill
// listener.
//
// Declare GRPCAdapter fields in a component implementation alongside a
// twill.Listener. The field must have a twill tag with listener, service, and
// method entries:
//
//	type legacy struct {
//	    twill.Implements[Legacy]
//	    grpc       twill.Listener
//	    getProfile twill.GRPCAdapter `twill:"listener=grpc,service=profiles.v1.ProfileService,method=GetProfile"`
//	}
//
// The marker is used by twill generate and local app context. Use
// runtime/adapters to register existing gRPC services on a Twill listener.
type GRPCAdapter struct{}

// WithConfig[T] is a type that can be embedded inside a component
// implementation. The Twill runtime will take per-component
...
//
// To associate a router with its component, embed [twill.WithRouter] in the
// component implementation.
//
//	type lruCache struct {
//		twill.Implements[Cache]
//		twill.WithRouter[cacheRouter]
//	}
//
...
//	struct{twill.AutoMarshal; x int; y string}
//
...
// # Semantics
//
// NOTE that routing is done on a best-effort basis. Twill will try to
...
// "twill generate" should generate serialization methods for the struct.
//
...
//	}
//
// The AutoMarshal embedding instructs "twill generate" to generate
...
// For example, "twill generate" will raise an error for the following code
